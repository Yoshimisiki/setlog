...
```
フロントエンド:  Next.js 16.1.6 (App Router) + TypeScript 5 + React 19
...
type SetlistItem = {
  ...
  deezer_id?: string        // Deezer取得時のみ（odesliに渡す用 + Deezerページリンク）
...
image:layout           → 画像カスタマイザーレイアウト（logoX, logoY, logoSize, qrX, qrY, textX, textY）
                          ※ textY は未使用（テキストブロックはX方向のみ移動可）
image:bg               → 背景画像（DataURL）
band:logo              → バンドロゴ画像（DataURL）
setlog:add-to-home-screen-hint-dismissed → '1' で次回以降A2HS案内を非表示
```
...
#### 実装済み日本語コピー（src/app/page.tsx）

- H1: セトリ作りを、もっと楽しく。
- サブコピー: バンド名と持ち時間を入れるだけ。曲・MC・SEを並べて、残り時間に合う曲をSETLOGが自動生成します。終演後のセトリ画像も、アー写・フライヤー・ロゴ・フォントを選んでそのまま作れます。
- バッジ: 登録不要・無料
- CTA: セトリを作る →
- 機能見出し: 持ち曲から、思いがけないセトリを作る。
- カード1: 持ち曲でセトリを自動生成
- カード2: 気に入った曲は固定
- カード3: MC・SEも一緒に管理
- カード4: セトリ画像をその場で作成
- ストリーミング説明: 検索リンクを自動表示（「連携」表現は使用しない）
- ホーム画面案内: 共有ボタンから「ホーム画面に追加」すると、アプリのように使えます。
- 下部CTA: 削除済み
