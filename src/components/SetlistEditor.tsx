...
export default function SetlistEditor({ initialSetlist, initialBandName }: Props) {
  ...
  // 自動生成済み（または旧データの iTunes 由来曲）かどうかを判定
  const isAutoGeneratedItem = (item: SetlistItem): boolean => {
    if (item.generated) return true
    if (item.type !== 'song') return false
    if (item.source === 'manual') return false
    if (item.apple_music_url || item.preview_url) return true
    return false
  }

  const handlePinGenerated = (id: string) => {
    const items = setlist.items.map(item =>
      item.id === id ? { ...item, generated: false } : item
    )
    update({ ...setlist, items })
    toast.success(t('editor.pinGeneratedSongSuccess'))
  }

  const hasGeneratedItems = setlist.items.some(item => item.generated === true)

  const clearGeneratedSongs = () => {
    if (!window.confirm(t('editor.clearGeneratedSongsConfirm'))) return
    const items = setlist.items.filter(item => item.generated !== true)
    update({ ...setlist, items })
    toast.success(t('editor.clearGeneratedSongsSuccess'))
  }

  const shuffleTracks = (tracks: ITunesTrack[]) => {
    const arr = [...tracks]
  ...
  const buildCandidate = (
    ...
    fixedTitleKeys: Set<string>,
    isInfinite: boolean
  ): { items: Omit<SetlistItem, 'id'>[]; total: number } => {
    const shuffled = shuffleTracks(tracks)
    const titleKeys = new Set(fixedTitleKeys)
    const items: Omit<SetlistItem, 'id'>[] = []
    let total = 0
    for (const track of shuffled) {
      const normalizedTitle = normalizeSongTitle(track.trackName)
      if (!normalizedTitle) continue
      if (titleKeys.has(normalizedTitle)) continue
      const dur = Math.round((track.trackTimeMillis ?? 0) / 1000)
      if (!dur || dur <= 0) continue
      if (!isInfinite && total + dur > remainingSeconds) continue
      items.push({
        type: 'song',
  ...
  const autoGenerate = async () => {
    const fixedItems = setlist.items.filter(item => !isAutoGeneratedItem(item))
    ...
    try {
      ...
      // 複数回試行して残り時間に最も近い組み合わせを採用
      const attempts = isInfinite ? 1 : 50
      let best: { items: Omit<SetlistItem, 'id'>[]; total: number } = { items: [], total: 0 }
      for (let i = 0; i < attempts; i++) {
        const candidate = buildCandidate(allTracks, remainingSeconds, fixedTitleKeys, isInfinite)
        if (candidate.total > best.total) best = candidate
      }

      if (best.items.length === 0) {
        toast.error(t('editor.autoGenerateNoFittingSongs'))
        return
      }

      const newItems: SetlistItem[] = best.items.map(item => ({ ...item, id: nanoid() }))
    ...
    <main className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        ...
        <div className="border-t border-border p-3 flex flex-wrap gap-2">
          ...
          {hasGeneratedItems && (
            <Button size="sm" variant="outline"
              className="border-border bg-secondary/40 text-muted-foreground hover:bg-secondary hover:border-primary hover:text-foreground ml-auto"
              onClick={clearGeneratedSongs}
            >
              {t('editor.clearGeneratedSongs')}
            </Button>
          )}
        </div>
      </div>
